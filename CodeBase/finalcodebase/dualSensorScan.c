...
void calc_botPos(float rotation, float distance){
    ...
    bot_x = (distance * cos(rotation)) + bot_x;
    bot_y = (distance * sin(rotation)) + bot_y;
    bot_rotation = bot_rotation + rotation;

}

char calc_coord(object obj){
    ...
    float xCord = dist * cos(ang) + box_x;
    sprintf(coord, "%.2f, \t, %.2f", bot_x, bot_y);
