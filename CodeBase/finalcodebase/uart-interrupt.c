...
volatile char recieved_char = 0;
char command = ' ';
int dataCount = 0;
volatile char dataIn[50];


int receiving = FALSE;
int receivingData = FALSE;
int packetFinished = FALSE;
...
void uart_interrupt_init(void)
{
    ...
    GPIO_PORTB_DEN_R |= 0x03; // 0x03 for both Rx and Tx. TODO decide if we need both or just Rx
...
void uart_sendChar(char data)
{
    ...
    UART1_DR_R = data;
    recieved_char = data;
    // FIXME Ask TA's about this function and the next function
}
...
void UART1_Handler(void)
{
    ...
    if (UART1_MIS_R & 0x10)
    {
        ...
        //waits for the first _
        if(byte_received == 'h' && receiving == FALSE && packetFinished == FALSE){
            uart_sendChar('h');

        }
        else if(byte_received == '_' && receiving == FALSE)
        {
            receiving = TRUE;
        }
        else if(receiving == TRUE)
        {
            //looks for the first character after the first _
            if(byte_received != '_' && receivingData == FALSE){
                command = byte_received;
            }
            //looks for the second _
            else if (byte_received == '_' && receivingData == FALSE)
            {
                receivingData = TRUE;
                //resets the data string to take in new data
                dataCount = 0;
            }
            //saves data until third _
            else if (receivingData == TRUE && byte_received != '_'){
                //python is only sending the first 4 characters for some fucking reason
                dataIn[dataCount] = byte_received;
                dataCount++;
            //resets everything and begins looking for the first _ from the next packet
            } else {
                packetFinished = TRUE;
                dataIn[dataCount] = '\0';
                dataCount = 0;
        ...
        {
        // waits until header is finished
        if(packetFinished == TRUE){
