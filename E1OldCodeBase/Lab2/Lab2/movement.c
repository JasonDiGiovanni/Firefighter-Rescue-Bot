...
double m_forward(oi_t*sensor_data, double distance_mm);
...
double move_forward(oi_t*sensor_data, double distance_mm){
  ...
  while(sum < distance_mm){//go untill distance
      ...
      if( sensor_data -> bumpLeft){
          ...
          move_backward(sensor_data, 150);//same as below but 150
          turn_right(sensor_data, 90);
          m_forward(sensor_data, 250);//this causes my distance to change to 250 wtf do i do about that
          turn_left(sensor_data, 90);
      }

      if(sensor_data -> bumpRight){
          oi_update(sensor_data);
               move_backward(sensor_data, 150);//same as above but 150
               turn_left(sensor_data, 90);
                m_forward(sensor_data, 250);//same as above 250
